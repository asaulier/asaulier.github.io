...
function main_curve1(countries_codes, raw, width=450 ,height=250){
	let raw_price = raw.map(function nimportequoi(d){
	  return Object.assign( {}, d.price, {"date":d.date}) //merge the two dictionnaries
	});
   ...
   return plot_curve_index_price(raw_price, get_curve_selection(countries_codes))
	...
	function plot_curve_index_price(raw_dataset, list_of_curves){
	  ...
	  // WE HAVE TO TAKE ACCOUNT OF THE CHANGE BETWEEN BOTH VARIABLES
		...
		var list_of_first_values = [] ; for (const key of list_of_curves){ list_of_first_values.push(dataset_of_datapoints[0][key])}
		for (const number_curve of list_of_curves.keys()){
			dataset.push( dataset_of_datapoints.map(d => {let res={}; const v =list_of_first_values[number_curve]; 
		...
		for (const name_curve of list_of_curves){
			  dataset.push( dataset_of_datapoints.map(d => { let res={}; if(d[name_curve] ==0)// if data missing (here it means =0)
