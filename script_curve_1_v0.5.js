...
function main_curve1(countries_codes, raw, list_c = undefined, colorate = undefined){
	// raw = [ [ {date : ?, action = ?}] , [], [] ]
	console.log("Executing main_curve1 ... hereafter is the raw dataset")
	console.log(raw)
	let list_of_curves = list_c;
	let coloration = colorate;
	//console.log(raw)
	//sizing the divs
	let dimensions = get_div_dimension("div_curve_1");
	...
	var relatif = 0;
	if(d3.select("#rel_abs1").node() != null){
		relatif = (d3.select("#rel_abs1").node().value == "Relatif") + (d3.select("#rel_abs1").node().value == "day-day %")*2;
	}
	...
	if(dataset != [] ){
		...
		if(true_dataset.length !=0){
			...
			return plot_line(d3.select("#curve_for_map"), width, height, true_dataset, true_list_of_curves ,true_coloration,0, {"y":titre_axe_y})
			}
		}
	}
}